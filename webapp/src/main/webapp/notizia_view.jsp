...
<head>
    <title>BNCF - <%=titolo%> / <%=autore%></title>
...
<c:choose>
 <c:when test="${empty dom}"><%=lang.translate("nessun_documento") %></c:when>
 <c:otherwise>
  <div id="notizia">
<%
  String cart = request.getParameter("cart");
  if ((cart==null) || !cart.equals("true")) {
%>
<%@ include file="inc/notizia-nav.jsp" %>
<% } else { %>
<%@ include file="inc/notizia-cart-nav.jsp" %>
<% } %>
...
<br/>
  <div id="links">
   <div class="links-left">
      <a href="?fmt=txt" style="margin-right:1em;"
	title="<%=lang.translate("visualizza_record_formato_txt")%>"><img
		src="<%=contextPath%>/img/txt_download.png"
		alt="<%=lang.translate("visualizza_record_formato_txt")%>"/></a>

      <a href="?fmt=xml" style="margin-right:4em;"
	title="<%=lang.translate("visualizza_record_formato_xml")%>"><img
		src="<%=contextPath%>/img/xml_download.png"
		alt="<%=lang.translate("visualizza_record_formato_xml")%>"/></a>

      <a href="javascript://" id="open-close-suggest" style="margin-right:3em;"
	title="<%=lang.translate("aggiungi_suggerimento") %>"
	onclick="$.facebox({ ajax: '<%=contextPath%>/inc/suggest.jsp?bid=<%=currentBid %>' }); return false;"><img
		src="<%=contextPath%>/img/add_suggest.png"
		alt="<%=lang.translate("aggiungi_suggerimento") %>"
	...
	id="add-<%=currentBid%>-to-cart" class="add-to-cart" title="<%=lang.translate("aggiungi_doc_al_carrello")%>"
	onclick="addBidToCart('<%=currentBid%>','<%=tit%>','<%=aut%>','<%=anno%>'); return false;"
	><img style="vertical-align: bottom;" src="<%=contextPath%>/img/add-book.png"
		alt="<%=lang.translate("aggiungi_doc_al_carrello")%>" /></a>

      <a href="javascript://" <%if(!bidInCart){%>style="display: none;"<%}%>
	id="remove-<%=currentBid%>-to-cart" class="remove-to-cart" title="<%=lang.translate("rimuovi_doc_dal_carrello")%>"
	onclick="removeBidFromCart('<%=currentBid%>'); return false;"
	><img style="vertical-align: bottom;" src="<%=contextPath%>/img/added-book.png"
		alt="<%=lang.translate("rimuovi_doc_dal_carrello")%>"/></a>
   ...
   <div class="addthis_toolbox addthis_default_style links-right"
	addthis:url="http://opac.bncf.firenze.sbn.it/bid/<%=currentBid%>"
	addthis:title="<%=tit%>"
	addthis:description="<%=aut%>, <%=anno%>">
	...
	class="addthis_button_compact"><%=lang.translate("share") %></a>
   </div>
   <script type="text/javascript"
