...
<xsl:template match="df[@t=225]">
  ( <i><a><xsl:attribute name="href">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:value-of select="substring(/rec/df[@t='410']/s1/cf[@t='001'],1,10)"/>
     </xsl:attribute>
     <xsl:attribute name="title">Ricerca tutte le notizie in questa collana</xsl:attribute>
...
<xsl:template match="df[@t=430]/s1/df[@t=200]" >
  ...
  &br1;
  <a><xsl:attribute name="href">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:value-of select="substring(../../s1/cf[@t=001],1,10)"/></xsl:attribute>    <xsl:attribute name="title">Ricerca tutte le notizie con questo titolo</xsl:attribute>
...
<xsl:template match="df[@t=440]/s1/df[@t=200]" >
  ...
  &br1;
  <a><xsl:attribute name="href">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:value-of select="substring(/rec/df[@t=440]/s1/cf[@t=001],1,10)"/></xsl:attribute>    <xsl:attribute name="title">Ricerca tutte le notizie con questo titolo</xsl:attribute>
  <xsl:for-each select="sf">
	<xsl:if test="@c='a'">
	 <xsl:call-template name="substDelim">
           <xsl:with-param name="string"><xsl:value-of select="."/></xsl:with-param>
...
<xsl:template name="supplementoa" >
 ...
 <xsl:for-each select="df[@t=422]">
  ...
  <xsl:for-each select="s1/df[@t=200]">
   &br1;
   <a><xsl:attribute name="href">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:value-of select="$idn"/></xsl:attribute>    <xsl:attribute name="title">Ricerca tutte le notizie con questo titolo</xsl:attribute>
...
<xsl:template name="continuatoparziale" >
 ...
 <continuatoeparziale>
 &br2;
 <b>Continuato parzialmente da:</b>
...
<xsl:template name="assorbito" >
 ...
 <xsl:for-each select="df[@t=444]">
  ...
  <xsl:for-each select="s1/df[@t=200]">
   &br1;
   <a><xsl:attribute name="href">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:value-of select="$idn"/></xsl:attribute>
	<xsl:if test="sf[@c='v']"><xsl:value-of select="sf[@c='v']"/> : </xsl:if>
	<xsl:call-template name="substDelim">
           <xsl:with-param name="string"><xsl:value-of select="sf[@c='a']"/></xsl:with-param>
	</xsl:call-template>
...
<xsl:template match="df[@t=461]/s1/df[@t=200]" >
  <a><xsl:attribute name="href">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:value-of select="substring(/rec/df[@t='461']/s1/cf[@t='001'],1,10)"/> 
     </xsl:attribute>
    <xsl:attribute name="title">Ricerca tutte le notizie con questo titolo</xsl:attribute>
	<xsl:call-template name="substDelim">
	  <xsl:with-param name="string"><xsl:value-of select="sf[@c='a']"/></xsl:with-param>
	</xsl:call-template>
	<xsl:if test="sf[@c='e']"> : <xsl:value-of select="sf[@c='e']"/> </xsl:if>
...
-->
   <xsl:variable name="ricman"><xsl:if test="($codbib!='CF') or ($segn3 = 'GF ') or ($segn4='CONS') or ($segn4='S M ') or ($segn4='S L ') or ($segn4='F ST')">1</xsl:if></xsl:variable>
   ...
   <xsl:variable name="pos" select="count(../preceding-sibling::*)"/>
	...
	<tr>
		...
		<a> <xsl:attribute name="href"
			...
			<xsl:attribute name="title">Apri il box con il dettaglio annate</xsl:attribute>
			<img>
			<xsl:attribute name="align">absmiddle</xsl:attribute>
			<xsl:attribute name="hspace">2</xsl:attribute>
			<xsl:attribute name="id"><xsl:value-of select="$dettid"/>_img</xsl:attribute>
			<xsl:attribute name="src">img/folderclose.gif</xsl:attribute>
			</img>Dettaglio annate
		</a>
		</td>
	</tr>
...
-->
  <a><xsl:attribute name="title">Cliccare per richiedere le annate</xsl:attribute><xsl:attribute name="href">http://richieste.bncf.firenze.sbn.it/Richieste/servlet/Richieste?BIB=%20CF&amp;amp;INV=1%20%20<xsl:value-of select="$richinv"/>&amp;amp;AUTO=Y&amp;amp;BID=<xsl:value-of select="$bid"/><xsl:if test="$annoed">&amp;amp;NAT=S&amp;amp;ANNO=<xsl:value-of select="$annoed"/></xsl:if>&amp;amp;<xsl:if test="not($aut='')">AUT=<xsl:value-of select="$aut"/>&amp;amp;</xsl:if>TIT=<xsl:value-of select="$tit"/></xsl:attribute><xsl:value-of select="$label"/></a>
...
<xsl:template match="df[@t=960]/sf[@c='e']" mode="monografia">
 ...
 <xsl:choose>
  <xsl:when test="$codbib='CF'">
	...
	<!-- <xsl:when test="$disp='A '"><xsl:value-of select="$invm"/> (<a><xsl:attribute name="href">inlavorazione.jsp</xsl:attribute>disponibilit&agrave; limitata</a>)</xsl:when> -->
	<xsl:when test="$disp='A '"><xsl:value-of select="$invm"/> (non disponibile)</xsl:when>
  ...
  <xsl:otherwise><xsl:value-of select="$invm"/></xsl:otherwise>
 </xsl:choose>
		</td>
	</tr>
...
-->
  <a><xsl:attribute name="title">Clicca per richiedere il documento</xsl:attribute><xsl:attribute name="href">http://richieste.bncf.firenze.sbn.it/Richieste/servlet/Richieste?BIB=%20<xsl:value-of select="$codbib"/>&amp;amp;INV=1%20%20<xsl:value-of select="$richinv"/>&amp;amp;AUTO=Y&amp;amp;BID=<xsl:value-of select="$bid"/><xsl:if test="$annoed">&amp;amp;NAT=M&amp;amp;ANNO=<xsl:value-of select="$annoed"/></xsl:if>&amp;amp;AUT=<xsl:call-template name="autore_breve"/>&amp;amp;TIT=<xsl:value-of select="$tit"/></xsl:attribute><xsl:value-of select="$invm"/></a>
...
Gli utenti registrati possono accedere ai servizi della biblioteca scegliendo il numero di inventario del documento desiderato.
<i>Controllare <a><xsl:attribute name="href">modalita.jsp</xsl:attribute><xsl:attribute name="title">Controlla le modalit&agrave; per richiedere il documento</xsl:attribute>modalit&agrave;</a> e <a><xsl:attribute name="href">http://www.bncf.firenze.sbn.it/orario.php</xsl:attribute><xsl:attribute name="title">Controlla l'orario per richiedere il documento</xsl:attribute>orario</a> dei servizi</i>.
</xsl:when>
<xsl:when test="$natura = 's'">
Gli utenti registrati possono accedere al documento cliccando su "Annate possedute" e scegliendo l'anno del periodico desiderato fra quelli posseduti dalla Biblioteca.
...
-->
    ...
    <tr><td><xsl:attribute name="nowrap">1</xsl:attribute>
	 ...
	 <xsl:value-of select="$colloc"/>
      ...
      <xsl:choose>
      <xsl:when test="$segn3 = 'INF'">Ufficio Informazioni: <xsl:value-of select="substring($coll,4,string-length($coll)-3)"/></xsl:when>
      <xsl:otherwise><xsl:value-of select="$coll"/></xsl:otherwise>
      </xsl:choose>
      <xsl:if test="$ricman = 1"> (richiesta manuale)</xsl:if>
...
-->
  ...
  <xsl:choose>
   <xsl:when test="$ricman = 1">Annate&nbsp;possedute:&nbsp;</xsl:when>
   <xsl:when test="contains($segn4,'CONS') or contains($segn4,'S M ') or contains($segn4,'S L ') or contains($segn4,'F ST')">Annate&nbsp;possedute:&nbsp;</xsl:when>
<!--
   <xsl:otherwise><a><xsl:attribute name="title">Cliccare per richiedere le annate</xsl:attribute><xsl:attribute name="href">http://servizi.bncf.firenze.sbn.it/richieste/Servlet/Pubblica?BIB=%20CF&amp;amp;INV=1%20%20<xsl:value-of select="$richinv"/>&amp;amp;AUTO=Y&amp;amp;BID=<xsl:value-of select="$bid"/><xsl:if test="$annoed">&amp;amp;NAT=S&amp;amp;ANNO=<xsl:value-of select="$annoed"/></xsl:if>&amp;amp;<xsl:if test="not($aut='')">AUT=<xsl:value-of select="$aut"/>&amp;amp;</xsl:if>TIT=<xsl:value-of select="$tit"/></xsl:attribute>Annate&nbsp;possedute:&nbsp;</a></xsl:otherwise>
-->
   <xsl:otherwise><a><xsl:attribute name="title">Cliccare per richiedere le annate</xsl:attribute><xsl:attribute name="href">http://richieste.bncf.firenze.sbn.it/Richieste/servlet/Richieste?BIB=%20CF&amp;amp;INV=1%20%20<xsl:value-of select="$richinv"/>&amp;amp;AUTO=Y&amp;amp;BID=<xsl:value-of select="$bid"/><xsl:if test="$annoed">&amp;amp;NAT=S&amp;amp;ANNO=<xsl:value-of select="$annoed"/></xsl:if>&amp;amp;<xsl:if test="not($aut='')">AUT=<xsl:value-of select="$aut"/>&amp;amp;</xsl:if>TIT=<xsl:value-of select="$tit"/></xsl:attribute>Annate&nbsp;possedute:&nbsp;</a></xsl:otherwise>
  </xsl:choose>
      ...
      <xsl:if test="$bibnaz != 1">
      <td><xsl:attribute name="nowrap">1</xsl:attribute>Annate&nbsp;possedute:&nbsp;</td>
      </xsl:if>
 ...
 <!-- </xsl:if> --> <!-- annate possedute -->
   ...
   <tr>
    <td><xsl:attribute name="colspan">2</xsl:attribute>
      <a><xsl:attribute name="href">javascript:ExpandCollapse('dettagli<xsl:value-of select="$pos"/>')</xsl:attribute>     <xsl:attribute name="title">Apri il box con il dettaglio annate</xsl:attribute>
	<img>
	<xsl:attribute name="align">absmiddle</xsl:attribute>
	<xsl:attribute name="hspace">2</xsl:attribute>
	<xsl:attribute name="id">dettagli<xsl:value-of select="$pos"/>_img</xsl:attribute>
	<xsl:attribute name="src">img/folderclose.gif</xsl:attribute></img>Dettaglio annate
	</a>
    </td>
   </tr>
...
<!--
	<xsl:otherwise><a><xsl:attribute name="title">Clicca per richiedere il documento</xsl:attribute><xsl:attribute name="href">http://servizi.bncf.firenze.sbn.it/richieste/Servlet/Pubblica?BIB=%20<xsl:value-of select="$bib"/>&amp;amp;INV=1%20%20<xsl:value-of select="$inv"/>&amp;amp;AUTO=Y&amp;amp;BID=<xsl:value-of select="$bid"/><xsl:if test="$annoed">&amp;amp;NAT=M&amp;amp;ANNO=<xsl:value-of select="$annoed"/></xsl:if>&amp;amp;AUT=<xsl:call-template name="autore_breve"/>&amp;amp;TIT=<xsl:value-of select="$tit"/></xsl:attribute><xsl:value-of select="$invm"/><xsl:if test="$anno">&nbsp;&nbsp;<xsl:value-of select="$anno"/></xsl:if></a>
-->
	<xsl:otherwise><a><xsl:attribute name="title">Clicca per richiedere il documento</xsl:attribute><xsl:attribute name="href">http://richieste.bncf.firenze.sbn.it/Richieste/servlet/Richieste?BIB=%20<xsl:value-of select="$bib"/>&amp;amp;INV=1%20%20<xsl:value-of select="$inv"/>&amp;amp;AUTO=Y&amp;amp;BID=<xsl:value-of select="$bid"/><xsl:if test="$annoed">&amp;amp;NAT=M&amp;amp;ANNO=<xsl:value-of select="$annoed"/></xsl:if>&amp;amp;AUT=<xsl:call-template name="autore_breve"/>&amp;amp;TIT=<xsl:value-of select="$tit"/></xsl:attribute><xsl:value-of select="$invm"/><xsl:if test="$anno">&nbsp;&nbsp;<xsl:value-of select="$anno"/></xsl:if></a>
	</xsl:otherwise>
	</xsl:choose>
		...
		<xsl:if test="$displim = 1">
		  <xsl:value-of select="$invm"/><xsl:if test="$anno">&nbsp;&nbsp;<xsl:value-of select="$anno"/></xsl:if> (<a><xsl:attribute name="href">inlavorazione.jsp</xsl:attribute>disponibilita limitata</a>)
		</xsl:if>
	...
	<xsl:when test="$type = 'A'">
		<xsl:value-of select="$invm"/>&nbsp;&nbsp;<xsl:value-of select="$anno"/>Mancante
	</xsl:when>
	...
	<xsl:otherwise>
		<xsl:choose>
		<xsl:when test="$bibnaz = 1">
		   (<a><xsl:attribute name="href">inlavorazione.jsp</xsl:attribute><xsl:attribute name="title">Richiedi il documento</xsl:attribute>Disponibilit&agrave; limitata</a>)
		</xsl:when>
		<xsl:otherwise>(Disponibilit&agrave; limitata)</xsl:otherwise>
		</xsl:choose>
	</xsl:otherwise>
      </xsl:choose>
	&br1;
...
<xsl:if test="position()>1">,&nbsp;&nbsp;&nbsp;</xsl:if>
<a><xsl:attribute name="title">Cliccare per visualizzare i fascicoli</xsl:attribute>
	<xsl:attribute name="href">controller?action=sol_viewabbo&amp;sol_abbonamento=<xsl:value-of select="$abbo"/>&amp;sol_biblioteca=<xsl:value-of select="$codbib"/></xsl:attribute>Abb.&nbsp;<xsl:value-of select="$abbo"/></a>
</xsl:for-each>
</xsl:template>
...
<xsl:template match="df[@t=102]/sf[@c='a']" >
 <paese>
  ...
  <xsl:choose>
	...
	<xsl:when test=".='NL'">Olanda</xsl:when>
	<xsl:when test=".='CH'">Swizzera</xsl:when>
	<xsl:when test=".='BE'">Belgio</xsl:when>
	<xsl:when test=".='HU'">Ungheria</xsl:when>
...
<xsl:template match="df[@t=101]" >
   ...
   <xsl:for-each select="sf[@c='a']|sf[@c='b']">
    ...
    <xsl:choose>
    <xsl:when test="$lang='ita'">italiano</xsl:when>
    <xsl:when test="$lang='eng'">inglese</xsl:when>
    <xsl:when test="$lang='france'">francese</xsl:when>
    <xsl:when test="$lang='fre'">francese</xsl:when>
    <xsl:when test="$lang='spa'">spagnolo</xsl:when>
    <xsl:when test="$lang='ger'">tedesco</xsl:when>
    <xsl:when test="$lang='grc'">greco</xsl:when>
    <xsl:when test="$lang='por'">portoghese</xsl:when>
    <xsl:when test="$lang='lat'">latino</xsl:when>
    <xsl:when test="$lang='rus'">russo</xsl:when>
    <xsl:when test="$lang='mul'">multilingua</xsl:when>
...
<xsl:template name="classi" >
 ...
 <xsl:for-each select="df[@t=676]">
   ...
   <a>	<xsl:attribute name="href">controller?action=search_bydeweysearch&amp;amp;query_fieldname_1=coddewey&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='a']"/>
	</xsl:attribute>
	<xsl:attribute name="title">Ricerca tutte le notizie appartenenti a questa classe Dewey</xsl:attribute>
	<xsl:call-template name="subfieldSelect">
		<xsl:with-param name="codes">a</xsl:with-param>
		<xsl:with-param name="delimiter"> - </xsl:with-param>
	</xsl:call-template>
...
<xsl:template name="codmateria" >
 <xsl:for-each select="df[@t=686]">
  <xsl:if test="sf[@c='9'] or sf[@c='a']">&br1;Cod. materia:
...
<xsl:template match="df[@t=606]" >
 <soggetto>
  ...
  <a><xsl:attribute name="href">
      <xsl:choose>
       <xsl:when test="sf[@c='3']">controller?action=search_bysoggettosearch&amp;amp;query_fieldname_1=cidtutti&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='3']"/>
      </xsl:when>
       <xsl:otherwise><xsl:variable name="soggurl"><xsl:call-template name="subfieldSelect"><xsl:with-param name="codes">a</xsl:with-param><xsl:with-param name="delimiter"> </xsl:with-param></xsl:call-template></xsl:variable>controller?action=search_bysoggettosearch&amp;amp;query_fieldname_1=soggettonorm&amp;amp;query_querystring_1=<xsl:value-of select="translate($soggurl,'&lt;&gt;','')"/>
      </xsl:otherwise>
      </xsl:choose>
      </xsl:attribute>
      <xsl:attribute name="title">Ricerca tutte le notizie con questo soggetto</xsl:attribute>
...
<!--
  <xsl:if test="(sf[@c='2']='NFI') or (sf[@c='2']='nsogi')"> (Nuovo Soggettario)</xsl:if>
-->
  <xsl:text> &#160; </xsl:text>
 </soggetto>
</xsl:template>
...
<xsl:template name="responsec" >
 ...
 <xsl:for-each select="df[@t=701]|df[@t=702]|df[@t=712]">
  ...
  <xsl:if test="position() > 1">&nbsp;&nbsp;</xsl:if>
    ...
    <a><xsl:attribute name="href">
      <xsl:choose>
       <xsl:when test="sf[@c='3']">controller?action=search_byautoresearch&amp;amp;query_fieldname_1=vidtutti&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='3']"/>
      </xsl:when>
       <xsl:otherwise>
         <xsl:variable name="url"><xsl:value-of select="sf[@c='a']"/><xsl:text> </xsl:text><xsl:call-template name="subfieldSelect"><xsl:with-param name="codes">c</xsl:with-param></xsl:call-template><xsl:value-of select="sf[@c='b']"/></xsl:variable>controller?action=search_byautoresearch&amp;amp;query_fieldname_1=autorenorm&amp;amp;query_querystring_1=<xsl:value-of select="translate($url,'&lt;&gt;','')"/>
      </xsl:otherwise>
      </xsl:choose>
      </xsl:attribute>
      <xsl:attribute name="title">Ricerca tutte le notizie di quest'autore</xsl:attribute>
      <xsl:call-template name="subfieldSelectVirg">
  ...
  <xsl:choose>
    <xsl:when test="sf[@c='4']=320">
	&nbsp;<i>[possessore]</i>&nbsp;<xsl:value-of select="sf[@c='5']"/>  </xsl:when>
    <xsl:when test="sf[@c='4']=390">
	&nbsp;[<i>provenienza</i>&nbsp;<xsl:value-of select="sf[@c='5']"/>] </xsl:when>
    <xsl:when test="sf[@c='4']=650">
	&nbsp;<i>[editore]</i>     </xsl:when>
  </xsl:choose>
  &nbsp;&nbsp;
 ...
 <xsl:variable name="C500_9" select="count(sf[@c='9'])"/>
  ...
  <xsl:choose>
	<xsl:when test="@t=500">Tit.: </xsl:when>
	<xsl:when test="@t=510">Tit. parallelo: </xsl:when>
	<xsl:when test="$C500_9=0">Titolo: </xsl:when>
	<xsl:when test="$C500_9=1">Titolo originale: </xsl:when>
  </xsl:choose>
  ...
  <xsl:choose>
   ...
   <xsl:otherwise>
    <a><xsl:attribute name="href">
      <xsl:choose>
       <xsl:when test="sf[@c='3']">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='3']"/>
      </xsl:when>
       <xsl:otherwise>controller?action=search_bytitolosearch&amp;amp;query_fieldname_1=titolonorm&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='a']"/><xsl:value-of select="sf[@c='b']"/><xsl:text> </xsl:text><xsl:value-of select="sf[@c='c']"/>
      </xsl:otherwise>
      </xsl:choose>
     </xsl:attribute>
     <xsl:attribute name="title">Ricerca tutte le notizie con questo titolo</xsl:attribute>
...
<xsl:template name="notetit">
 <xsl:for-each select="df[@t=500 and @i1=1 and @i2=0]">
   ...
   <!-- area note -->
     &br1;Titolo originale:
   <a><xsl:attribute name="href">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:call-template name="getNotetitBid"><xsl:with-param name="field" select="."/></xsl:call-template></xsl:attribute><xsl:call-template name="substDelim"><xsl:with-param name="string"><xsl:value-of select="sf[@c='a']"/></xsl:with-param></xsl:call-template></a>
   </xsl:if>
 </xsl:for-each>
</xsl:template>
...
<xsl:template name="titolo_orig" >
 ...
 <a><xsl:attribute name="href">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:value-of select="substring(s1/cf[@t='009'],1,10)"/><xsl:value-of select="substring(s1/cf[@t='001'],1,10)"/> 
     </xsl:attribute>
...
<xsl:template name="autore_breve" >
 ...
 <xsl:for-each select="/rec/df[@t=700]|/rec/df[@t=710]">
  ...
  <a><xsl:attribute name="href">controller?action=search_byautoresearch&amp;amp;query_fieldname_1=vidtutti&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='3']"/></xsl:attribute>
  </a>
  <a><xsl:attribute name="href">
      <xsl:choose>
       <xsl:when test="sf[@c='3']">controller?action=search_byautoresearch&amp;amp;query_fieldname_1=vidtutti&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='3']"/>
      </xsl:when>
       <xsl:otherwise>controller?action=search_byautoresearch&amp;amp;query_fieldname_1=autorenorm&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='a']"/><xsl:value-of select="sf[@c='b']"/>
      </xsl:otherwise>
      </xsl:choose>
     </xsl:attribute>
     <xsl:attribute name="title">Ricerca tutte le notizie di quest'autore</xsl:attribute>
 ...
 <xsl:for-each select="df[@t=500 and @i2='1']">
  ...
  <a><xsl:attribute name="href">
	...
	<xsl:when test="sf[@c='9']">controller?action=search_byidnsearch&amp;amp;query_fieldname_1=bidtutti&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='9']"/>
	</xsl:when>
	<xsl:otherwise>controller?action=search_bytitolosearch&amp;amp;query_fieldname_1=titolonorm&amp;amp;query_querystring_1=<xsl:value-of select="sf[@c='a']"/>
	</xsl:otherwise>
...
<xsl:template name="nationaluse">
 <xsl:if test="df[@t=790]|df[@t=791]|df[@t=900]|df[@t=910]">
 &br1;
 &br1;
 <nationaluse>
  <xsl:if test="df[@t=900]">
	...
	<xsl:if test="not($this = $prec)">
	  ...
	  <i> &egrave; la forma preferita per:</i>&br1;
	</xsl:if>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	   <xsl:call-template name="compattaPunct">
	     <xsl:with-param name="string">
  ...
  <xsl:if test="df[@t=910]">
	...
	<xsl:if test="not($this = $prec)">
	  <xsl:value-of select="$this"/> <i> &egrave; la forma preferita per:</i>&br1;
	</xsl:if>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	   <xsl:call-template name="compattaPunct">
	     <xsl:with-param name="string">
...
<div><xsl:attribute name="class">risdigit</xsl:attribute>
   <xsl:for-each select="df[@t=899]/sf[@c='u']|df[@t=956]/sf[@c='a']">
	...
	<xsl:choose>
	  ...
	  <xsl:otherwise>
	    <xsl:attribute name="href">javascript: loadArsbni('<xsl:value-of select="."/>');</xsl:attribute>
	  </xsl:otherwise>
	</xsl:choose>
	<xsl:choose>
	  <xsl:when test="../@i2='0'">
	    <xsl:attribute name="title">Accesso alla risorsa digitalizzata parzialmente</xsl:attribute>
	    <img src="img/ARS.jpg" border="0" align="center"/>&nbsp;Digitalizzazione parziale</xsl:when>
	  <xsl:when test="../@i2='1'">
	    <xsl:attribute name="title">Accesso alla copia digitale</xsl:attribute>
	    <img src="img/CD.jpg" border="0" align="center"/>&nbsp;Copia Digitale</xsl:when>
	 <xsl:when test="../@i2='2'">
	    <xsl:attribute name="title">Accesso alla copia depositata in BNCF</xsl:attribute>
	    <img src="img/ECPDEP.gif" border="0" align="center"/>&nbsp;Copia depositata in BNCF</xsl:when>
	</xsl:choose></A>&nbsp;&nbsp;&nbsp;
   </xsl:for-each>
