...
<xsl:template name="translate">
 ...
 <xsl:choose>
  <xsl:when test="$section='traduzioni'">
	<xsl:choose>
		<xsl:when test="$sigla='ricerca_notizie_autore'">Ricerca tutte le notizie di quest'autore</xsl:when>
		<xsl:when test="$sigla='ricerca_notizie_collana'">Ricerca tutte le notizie in questa collana</xsl:when>
		<xsl:when test="$sigla='ricerca_notizie_titolo'">Ricerca tutte le notizie con questo titolo</xsl:when>
		<xsl:when test="$sigla='ufficio_informazioni'">Ufficio Informazioni</xsl:when>
		<xsl:when test="$sigla='richiesta_manuale'">richiesta manuale</xsl:when>
		<xsl:when test="$sigla='annate_possedute'">Annate&nbsp;possedute&nbsp;</xsl:when>
		<xsl:when test="$sigla='apri_box_dettaglio_annate'">Apri il box con il dettaglio annate</xsl:when>
		<xsl:when test="$sigla='dettaglio_annate'">Dettaglio annate</xsl:when>
		<xsl:when test="$sigla='cliccare_richiedere_annate'">Cliccare per richiedere le annate</xsl:when>
		<xsl:when test="$sigla='cliccare_richiedere_documento'">Clicca per richiedere il documento</xsl:when>
		<xsl:when test="$sigla='cliccare_visualizzare_fascicoli'">Cliccare per visualizzare i fascicoli</xsl:when>
		<xsl:when test="$sigla='ricerca_notizie_dewey'">Ricerca tutte le notizie appartenenti a questa classe Dewey</xsl:when>
		<xsl:when test="$sigla='ricerca_notizie_soggetto'">Ricerca tutte le notizie con questo soggetto</xsl:when>
		<xsl:when test="$sigla='tit'">Tit.</xsl:when>
		<xsl:when test="$sigla='tit_parallelo'">Tit. parallelo</xsl:when>
		<xsl:when test="$sigla='titolo'">Titolo</xsl:when>
		<xsl:when test="$sigla='titolo_originale'">Titolo originale</xsl:when>
		<xsl:when test="$sigla='e_forma_preferita_per'">&egrave; la forma preferita per</xsl:when>
		<xsl:when test="$sigla='forma_preferita_per'">forma preferita per</xsl:when>
		<xsl:when test="$sigla='forma_preferita_per_nuovo_soggettario'">forma preferita con il Nuovo Soggettario (2007-) per</xsl:when>
		<xsl:when test="$sigla='altra_forma_per'">altra forma per</xsl:when>
		<xsl:when test="$sigla='accesso_risorsa_digitalizzata_parzialmente'">Accesso alla risorsa digitalizzata parzialmente</xsl:when>
		<xsl:when test="$sigla='digitalizzazione_parziale'">Digitalizzazione parziale</xsl:when>
		<xsl:when test="$sigla='copia_digitale'">Copia digitale</xsl:when>
		<xsl:when test="$sigla='accesso_copia_depositata_bncf'">Accesso alla copia depositata in BNCF</xsl:when>
		<xsl:when test="$sigla='copia_depositata_bncf'">Copia depositata in BNCF</xsl:when>
		<xsl:when test="$sigla='inventario'">Inventario</xsl:when>
...
<xsl:template match="df[@t=101]">
   ...
   <xsl:for-each select="sf[@c='a']|sf[@c='b']">
    ...
    <xsl:when test="$lang='fra'">francese</xsl:when>
